...
#print(wine)
wineLabels=['sepal length in cm','sepal width in cm','petal length in cm','petal width in cm']
wineLabels1=['sepal length in cm','sepal width in cm','petal length in cm','petal width in cm']
wineLabelsProperty=[1,1,1,1]
wineLabelsProperty1=[1,1,1,1]
#这是未对数值离散化的决策树
wineTree=trees.createTree(wine,wineLabels)
#这是对数值进行离散化的决策树
#wineTree=treescontinuous.createTree(wine,wineLabels,wineLabelsProperty)                    #必须要带上tree才不会报错
...
frtest=open('iris_test.txt')                                         #数据一定要保证标点符号符合规范，且最后不能有空行，否则会出现空列表导致运行出错
#lenses=[inst.strip().split('\t') for inst in fr.readline()]     #此处不能使用readline，否则返回的数据只有一行且分割为单个字母
